...
## Fish Audio API 利用制約（必須）

**Fish Audio APIは従量課金です。以下のルールを厳守してください：**

1. **音声生成（TTS）を実行する前に、必ずユーザーに承認を求める**
2. 承認なしでのAPI呼び出しは**絶対に禁止**
3. 承認時に以下を提示する:
   - 生成するテキストの内容と文字数
   - 推定コスト（目安: 1000文字あたり約$0.01-0.02）
4. ユーザーが「OK」「実行して」等の明確な承認を返した場合のみ実行

### 承認フロー例
```
秘書: 「Fish Audio APIで音声を生成します。
  - テキスト: 「2026年、新時代の投資戦略」（15文字）
  - 推定コスト: 約$0.01以下
  実行してよろしいですか？」
ユーザー: 「OK」
→ 実行
