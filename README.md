...
プロジェクトのフォルダをターミナルで開いた状態で、以下を実行してください。
...
> 既にgit initされているプロジェクトの場合は `bash ~/taisun-cc-company/setup.sh` だけでOKです。
...
| 2   | プロジェクト導入（プロジェクトを開いて `bash ~/taisun-cc-company/setup.sh`） | プロジェクトのターミナル             | プロジェクトごと          |
...
| 5   | 人事労務拡張パック（`bash ~/taisun-cc-company/setup-hr-labor-pack.sh`）       | プロジェクトのターミナル             | 人事労務開発時（任意）       |
| 6   | アップデート（`git pull` → `setup.sh`）※詳細は「アップデート手順」参照          | ターミナル（PowerShell / bash） | 新バージョン時           |
...
## ECC統合機能（everything-claude-code由来）

[everything-claude-code](https://github.com/affaan-m/everything-claude-code) のパフォーマンス最適化システムから3機能を統合しています。setup.shで自動セットアップされます。

| 機能 | 動作タイミング | 内容 |
|------|-------------|------|
| Session Memory Persistence | SessionStart / Stop | セッション状態の自動保存・復元。前回の進行中トピック・決定事項・未完了タスクを自動引き継ぎ |
| Continuous Learning | PostToolUse (Edit/Write/Bash) | ツール使用パターンを自動記録。3回以上+成功率80%でスキル化を提案 |
| Quality Gate | PostToolUse (Edit/Write) | コード変更後にconsole.log残留・debugger文・長い関数を自動検出 |

---

## Anthropic公式プラグイン

[claude-code公式リポジトリ](https://github.com/anthropics/claude-code/tree/main/plugins)から厳選した3プラグインを統合。setup.shで自動セットアップされます。

| プラグイン | 種類 | 内容 |
|-----------|------|------|
| /frontend-design | Skill | AIっぽい量産型UIを避け、記憶に残る独自デザインのフロントエンドを生成。タイポグラフィ・色・アニメーション・レイアウトの指針 |
| /code-review | Command | PRの自動レビュー。CLAUDE.md準拠チェック+バグ検出を並列エージェントで実行。高シグナル問題のみ報告 |
| security-guidance | Hook (PreToolUse) | Edit/Write時にコマンドインジェクション、XSS、eval、pickle等のセキュリティパターンを自動検出・警告 |

---

## 人事労務拡張パック（オプション）

人事・経理・労務システムの開発を支援するClaude Code環境拡張です。基盤セットアップ後に追加導入できます。
...
| 段 | 動作タイミング | 内容 |
|----|-------------|------|
| 第1段 | SessionStart（自動） | 料率マスタの鮮度チェック + 重要改定月アラート（1月:税制、3月:健保、4月:雇用保険、10月:最低賃金） |
| 第2段 | PostToolUse（自動） | 給与計算関連コード変更時にハードコード料率検出 + マイナンバーセキュリティ検査 |
| 第3段 | `/hr-law-check`（手動） | 厚労省・国税庁等からWebSearchで最新法改正を徹底リサーチ。公式情報の3重クロスチェック |

### 搭載コンポーネント

| 種類 | ファイル | 内容 |
|------|---------|------|
| Hook | hr-law-watcher.js | 法改正ウォッチャー（SessionStart） |
| Hook | hr-payroll-validator.js | 給与計算バリデータ（PostToolUse） |
| Rule | hr-domain-knowledge.md | 給与計算・社保・年調・入退社の業務ルール全書 |
| Rule | mynumber-security.md | マイナンバー安全管理措置（暗号化・アクセス制御・廃棄ルール） |
| Skill | /hr-law-check | 法改正リサーチスキル |
| Skill | /hr-rate-update | 料率マスタ初期設定・更新スキル |
| Data | rate-master.json | 社会保険料率・所得税・雇用保険等の一元管理マスタ |
