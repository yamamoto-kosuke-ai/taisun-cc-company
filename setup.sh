...
# ステップ1: 前提条件チェック
echo -e "${YELLOW}[1/16] 前提条件チェック...${NC}"
...
echo ""
echo -e "${YELLOW}[2/16] プロジェクト初期化...${NC}"
...
# ステップ3: シンボリックリンク作成
echo ""
echo -e "${YELLOW}[3/16] Taisun Agent シンボリックリンク作成...${NC}"
...
echo ""
echo -e "${YELLOW}[4/16] CC-Company統合スキルの注入...${NC}"
...
echo ""
echo -e "${YELLOW}[5/16] Playwright Skill の注入...${NC}"
...
echo ""
echo -e "${YELLOW}[6/16] プロンプトブースター & ブリーフィング を秘書に注入...${NC}"
...
echo ""
echo -e "${YELLOW}[7/16] Interactive Video Platform の注入...${NC}"
...
# ステップ8: Mission Skill の注入
echo ""
echo -e "${YELLOW}[8/16] Mission Skill の注入...${NC}"
...
# ステップ9: Briefing Skill の注入
echo ""
echo -e "${YELLOW}[9/16] Briefing Skill の注入...${NC}"
...
echo ""
echo -e "${YELLOW}[10/16] ECC統合 (everything-claude-code) セットアップ...${NC}"
...
echo ""
echo -e "${YELLOW}[11/16] Anthropic公式プラグイン注入...${NC}"
...
# ステップ: Anthropic公式スキル（Office文書+MCP Builder+Doc Coauthoring）注入
echo ""
echo -e "${YELLOW}[12/16] Anthropic公式スキル注入...${NC}"
...
for as_skill in docx xlsx pdf pptx mcp-builder doc-coauthoring skill-creator webapp-testing; do
    ...
    if [ -d "$AS_PACK_DIR/skills/$as_skill" ]; then
        mkdir -p "$AS_DEST"
        cp -rf "$AS_PACK_DIR/skills/$as_skill/"* "$AS_DEST/"
        echo -e "${GREEN}  ✅ /$as_skill${NC}"
    fi
done

echo -e "${BLUE}  出典: github.com/anthropics/skills${NC}"



# ステップ: Diagram Tools (Mermaid + D2) 注入
echo ""
echo -e "${YELLOW}[13/16] Diagram Tools 注入...${NC}"

DG_PACK_DIR="$SCRIPT_DIR/plugins/diagram-tools"

for dg_skill in mermaid-diagram d2-diagram; do
    DG_DEST="$TAISUN_HOME/.claude/skills/$dg_skill"
    mkdir -p "$DG_DEST"
    cp -f "$DG_PACK_DIR/skills/$dg_skill/SKILL.md" "$DG_DEST/SKILL.md"
    echo -e "${GREEN}  ✅ /$dg_skill${NC}"
done

echo -e "${BLUE}  Mermaid: フローチャート・ER図・ガントチャート等（GitHub埋込対応）${NC}"
echo -e "${BLUE}  D2: 高品質アーキテクチャ図・提案資料用（テーマ・手書き風対応）${NC}"

# ステップ5i: Skill Generator 注入
echo ""
echo -e "${YELLOW}[14/16] Skill Generator 注入...${NC}"
...
echo ""
echo -e "${YELLOW}[15/16] n8n MCP サーバー設定確認...${NC}"
...
if [ $ERRORS -eq 0 ]; then
    ...
    echo "  - Diagram Tools: /mermaid-diagram, /d2-diagram（フローチャート・アーキテクチャ図）"
